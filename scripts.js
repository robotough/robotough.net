...
const BORDER_TOGGLE_KEY = 'accentBorders'; // 'true' or 'false'
...
function initThemeSelector() {
    ...
    // 4. Accent Borders toggle: applies the accent color to container
    // borders/box-shadows sitewide instead of the default white. This runs
    // on every page (since initThemeSelector is called globally), but the
    // actual switch control only exists on the Color Selector page.
    const borderToggle = getElement('accentBorderToggle', false);

    function applyBorderToggle(enabled) {
        document.body.classList.toggle('accent-borders', enabled);
        localStorage.setItem(BORDER_TOGGLE_KEY, enabled ? 'true' : 'false');
        if (borderToggle) {
            borderToggle.checked = enabled;
        }
    }

    const savedBorderState = localStorage.getItem(BORDER_TOGGLE_KEY) === 'true';
    applyBorderToggle(savedBorderState);

    if (borderToggle) {
        borderToggle.addEventListener('change', (event) => {
            applyBorderToggle(event.target.checked);
        });
    }
}
